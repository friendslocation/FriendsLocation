...
async function renderReservations(rentals) {
    ...
    // allVehiclesData est déjà rempli par fetchVehicles
    if (allVehiclesData.length === 0) {
        await fetchVehicles(); // Assure que les véhicules sont chargés
    }
...
async function updateBookingStatus(rentalId, newStatus) {
    if (confirm(`Voulez-vous vraiment changer le statut de la réservation ${rentalId} à "${newStatus}" ?`)) {
        try {
            ...
            fetchRentals(); // Recharger les réservations pour mettre à jour l'affichage
        } catch (error) {
...
// --- Fonction d'impression de rapport (NOUVEAU) ---
printReportBtn.onclick = () => {
    // Créer une nouvelle fenêtre pour le rapport d'impression
    const printWindow = window.open('', '', 'height=600,width=800');
    printWindow.document.write('<html><head><title>Rapport des Réservations Friends Location</title>');
    // Inclure les styles de base pour l'impression
    ...
    printWindow.document.close(); // Fermer le document pour permettre le chargement complet
...
async function initializeAdminApp() {
    ...
    script.onload = async () => {
        ...
        // Charger les véhicules en premier pour qu'ils soient disponibles pour les réservations
        await fetchVehicles();
        // Puis charger les réservations qui dépendent des données des véhicules
        fetchRentals();
    };
